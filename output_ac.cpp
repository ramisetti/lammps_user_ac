  ...
  if (next_restart == ntimestep) {
    if (next_restart_single == ntimestep) {
      ...
      *ptr = '*';
      ...
      // delete the last but previous restart file
      int prev_step=ntimestep-2*restart_every_single;
      char *pt = strchr(restart1,'*');
      *pt = '\0';
      sprintf(file,"%s" BIGINT_FORMAT "%s",restart1,prev_step,pt+1);
      *pt = '*';      
      remove(file);
      // end of delete
