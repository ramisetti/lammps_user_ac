...
/* ----------------------------------------------------------------------
   ...
   Reference : http://www.uark.edu/ua/fengwang/DLPOLY2/node63.html
------------------------------------------------------------------------- */

#include <math.h>
#include <stdlib.h>
#include <string.h>
#include "pair_lj_cut_tip4p_smooth.h"
#include "atom.h"
#include "force.h"
#include "neighbor.h"
...
PairLJCutTIP4PSmooth::PairLJCutTIP4PSmooth(LAMMPS *lmp) : Pair(lmp)
{
  single_enable = 0;
  writedata = 1;
...
PairLJCutTIP4PSmooth::~PairLJCutTIP4PSmooth()
{
  if (allocated) {
...
void PairLJCutTIP4PSmooth::compute(int eflag, int vflag)
{
  int i,j,ii,jj,inum,jnum,itype,jtype;
  double qtmp,xtmp,ytmp,ztmp,delx,dely,delz,evdwl,ecoul;
  double rsq,r2inv,r6inv,forcecoul,forcelj,factor_lj,factor_coul,rinv;
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cut_coulsqplus) {
        ...
        if (rsq < cut_coulsq) {
          ...
          cforce = factor_coul * forcecoul * (r2inv-cut_coulsq_inv);
        ...
        // vlist stores 2,4,6 atoms whose forces contribute to virial
          ...
          if (eflag) {
			  ...
			  ecoul = qqrd2e * qtmp * q[j] * ( rinv - (2.0*cut_coul_inv) + (sqrt(rsq)*cut_coulsq_inv) );
            ecoul *= factor_coul;
          } else ecoul = 0.0;
...
void PairLJCutTIP4PSmooth::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairLJCutTIP4PSmooth::settings(int narg, char **arg)
{
  ...
  cut_coul_inv = 1.0/cut_coul;
  cut_coulsq_inv = 1.0/cut_coulsq;
  if (allocated) {
    int i,j;
...
void PairLJCutTIP4PSmooth::coeff(int narg, char **arg)
{
  if (narg < 4 || narg > 5)
...
void PairLJCutTIP4PSmooth::init_style()
{
...
double PairLJCutTIP4PSmooth::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
...
void PairLJCutTIP4PSmooth::write_restart(FILE *fp)
{
...
void PairLJCutTIP4PSmooth::read_restart(FILE *fp)
{
...
void PairLJCutTIP4PSmooth::write_restart_settings(FILE *fp)
{
...
void PairLJCutTIP4PSmooth::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void PairLJCutTIP4PSmooth::write_data(FILE *fp)
{
...
void PairLJCutTIP4PSmooth::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
...
void PairLJCutTIP4PSmooth::compute_newsite(double *xO,  double *xH1,
...
void *PairLJCutTIP4PSmooth::extract(const char *str, int &dim)
{
  dim = 0;
...
double PairLJCutTIP4PSmooth::memory_usage()
{
